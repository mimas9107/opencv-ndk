...
這份 plan 目前對應的是 OCR MVP 第一輪已驗證 checkpoint。
OCR 主流程已接通，P30 Pro 上已可辨識 `logitech`，preview 保護、權限流程、ROI gate 與節流調整都已完成第一輪收斂。
...
8. 加入 UI 顯示與結果去重
9. 實機調整閾值與性能
10. 實作影像擷取功能以收集訓練資料 (v0.2.6+)

## 11. 數據收集擴充 (v0.2.6+)

為了後續優化 CNN 模型或自行訓練更精準的 OCR 模型，需要建立數據收集機制：

- **目標**：捕捉「Preview 視野範圍」的原始影像。
- **技術選型**：
    - 直接從 `MainActivity` 的最新 `Bitmap` (由 JNI 渲染回傳者) 進行持久化。
    - 或是從 Native 層將處理後的 `cv::Mat` 直接編碼為 JPEG 儲存（效率較高）。
- **儲存規格**：
    - 格式：JPEG (Quality 95) 或 PNG。
    - 檔名：`OCR_TRAIN_YYYYMMDD_HHMMSS.jpg`。
- **UI 整合**：新增一個浮動按鈕 (FAB) 或簡單 Button 觸發「單次捕捉」。
